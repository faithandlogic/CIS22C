...
public class CountingGame {
    ...
    public static int doRhyme(ListInterface<Integer> players, ListInterface<String> rhyme, int startAt) {
        ...
        for (int i = 1; i <= rhyme.getLength(); i++) {
            System.out.println("Player " + players.getEntry(index) + " says: " + rhyme.getEntry(i));
            if (i == rhyme.getLength()) {
                System.out.println("Player " + players.getEntry(index) + " is out");
                players.remove(index);
            } else if (index == players.getLength()) {
                index = 1;
            } else {
                index++;
            }
